...
jQuery.fcbkListSelection = function(elem, width, height, row){
		...
		var addToSelected = function(obj){
				var val = parseInt($("#view_selected_count").text(), 10);
				if (obj.hasClass("itemselected")) {
						val--;
						$("#view_selected_count").text(val);
						obj.parents("li").removeAttr("addedid");
						obj.find("[type=hidden]").prop("disabled", true );
						if(val < 2) {
						...
						obj.find("[type=hidden]").prop("disabled", false );
						addValue(obj);
						if(val >= 2) {
		...
		var createTabs = function(elem, width){
				var html = '<div id="filters" style="width: 100%;">' + //(parseInt(width, 10) + 2) + 'px;">' +
		...
		var wrapElements = function(elem, width, height, row){
				...
				$(".fcbklist_item").css("width", "100%");   //newwidth + "px");
		}
