...
(function() {
	...
	app.Record = Backbone.Model.extend({
		...
		url: function() {
			return "/admin/faq/" + (this.isNew()? '' : this.id + "/");
		}
	});
	...
	app.HeaderView = Backbone.View.extend({
		...
		events: {
			...
			'keypress input[type="text"]': 'addNewOrEnter'/*,
		...
		addNewOrEnter: function(event) {
			if(event.keyCode !== 13) { return; }	// "Enter" key
			event.preventDefault();
			//this.addNew();
		}/*,
		addNew: function() {
			if(this.$el.find('[name="question"]').val())
		}*/
	});
	...
	app.ResultsView = Backbone.View.extend({
		...
		render: function() {
			...
			this.collcetion.each(function(record) {
	...
	app.ResultsRowView = Backbone.View.extend({
		...
		render: function() {
			this.$el.htm(this.template(this.model.attributes));
			return this;
		}
	});
